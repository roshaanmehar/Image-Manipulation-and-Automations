...
# -*- coding: utf-8 -*-
r"""
Hard-coded discrepancy checker

- CSV (same folder as this script): 1500_all_results.csv
...
CSV_BASENAME = "1500_all_results.csv"  # sits next to this script
...
def read_csv(csv_path: Path) -> Tuple[List[Dict[str, str]], str, str]:
...
def scan_root(root: Path) -> Tuple[Dict[str, str], List[str], Counter]:
    if not root.exists():
    ...
    return code_to_barcode, malformed, counts

def extract_csv_maps(rows: List[Dict[str, str]], code_header: str, barcode_header: str):
...
def main():
    ...
    folder_code_to_barcode, malformed, folder_dup_counts = scan_root(root)
    ...
    if extra_codes:
        write_csv(outdir / "extra_folders.csv",
                  [{"Code": c, "Folder Barcode": folder_code_to_barcode.get(c, "")} for c in extra_codes],
                  ["Code", "Folder Barcode"])
